...
1. **Security Check Job** (First Gate - Runs for all PRs except bot PRs)
   ...
   - Check contributor type (Internal org member vs External contributor)
...
3. **Run Tests - Internal Contributors** (Depends: validate-module)
   - If contributor is an internal org member
   - Checkout code + **simulate merge** on `module-tests` branch
   - Run Terraform tests
...
**Status**: DISABLED -- the trigger is commented out because this workflow
provisions live AWS infrastructure and no credentials are configured.
**Trigger (when re-enabled)**: Cron (Sunday 1AM UTC) OR manual dispatch
...
- Checks GitHub org membership for the repository owner org (`context.repo.owner`)
