...
violation[result] if {
	...
	result := {
		"policy": "terraform_module_github_only_policy",
		"severity": "error",
		"message": sprintf("Disallowed cloud provider detected: %s", [provider]),
		"details": sprintf("File %s contains reference to %s provider. Only GitHub is allowed among major cloud providers.", [file, provider]),
		"resolution": "Remove the disallowed provider and use GitHub resources instead",
	}
}
